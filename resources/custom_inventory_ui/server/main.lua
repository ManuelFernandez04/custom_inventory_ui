...
ESX.RegisterServerCallback("suku:getShopItems", function(source, cb, shoptype)
	...
	for i=1, #itemResult, 1 do
		...
		if shoptype == "musimundo" then
			for _, v in pairs(Config.Shops.MusiMundo.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'No tienes suficiente dinero!' })
					end
				end
            end
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'No tienes suficiente espacio en tu inventario!' })
        end
	end
				...
				if list[i].name == item then
					...
					if not targetWeapon then
						...
						if xPlayer.getMoney() >= totalPrice then
							...
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Compraste un'..list[i].label })
						else
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'No tienes suficiente dinero!' })
						end
					else
...
AddEventHandler('suku:buyLicense', function ()
	...
	if xPlayer.getMoney() >= Config.LicensePrice then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Registraste una licencia de Armas.' })
