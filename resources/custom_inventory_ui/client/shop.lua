...
Citizen.CreateThread(function()
    while true do
        ...
        if IsInRegularShopZone(coords) or IsInRobsLiquorZone(coords) or IsInYouToolZone(coords) or IsInMusiMundoZone(coords) or IsInPrisonShopZone(coords) or IsInWeaponShopZone(coords) then
            ...
            if IsInMusiMundoZone(coords) then
                if IsControlJustReleased(0, Keys["E"]) then
                    OpenShopInv("musimundo")
                    Citizen.Wait(2000)
                end
            end
            ...
            if IsInWeaponShopZone(coords) then
                if IsControlJustReleased(0, Keys["E"]) then
                    if Licenses['weapon'] ~= nil then
                        ...
                        exports['mythic_notify']:DoHudText('error', 'Necesitas una licencia de armas antes de poder comprar armas')
                    end
                end
            end
        end
...
function IsInMusiMundoZone(coords)
    MusiMundo = Config.Shops.MusiMundo.Locations
    for i = 1, #MusiMundo, 1 do
        if GetDistanceBetweenCoords(coords, MusiMundo[i].x, MusiMundo[i].y, MusiMundo[i].z, true) < 1.5 then
            return true
        end
    end
    return false
...
Citizen.CreateThread(function()
    while true do
        ...
        if GetDistanceBetweenCoords(coords, Config.WeaponLiscence.x, Config.WeaponLiscence.y, Config.WeaponLiscence.z, true) < 3.0 then
            ESX.Game.Utils.DrawText3D(vector3(Config.WeaponLiscence.x, Config.WeaponLiscence.y, Config.WeaponLiscence.z), "Presiona [E] para abrir la tienda", 0.6)

            if IsControlJustReleased(0, Keys["E"]) then
                if Licenses['weapon'] == nil then
                    OpenBuyLicenseMenu()
                else
                    exports['mythic_notify']:DoHudText('error', 'Ya tienes licencia de armas!')
                end
                Citizen.Wait(2000)
            end
        end
...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.Shops.RegularShop.Locations) do
        CreateBlip(vector3(Config.Shops.RegularShop.Locations[k].x, Config.Shops.RegularShop.Locations[k].y, Config.Shops.RegularShop.Locations[k].z ), "Supermercados", 3.0, Config.Color, Config.ShopBlipID)
    end

    for k, v in pairs(Config.Shops.RobsLiquor.Locations) do
        CreateBlip(vector3(Config.Shops.RobsLiquor.Locations[k].x, Config.Shops.RobsLiquor.Locations[k].y, Config.Shops.RobsLiquor.Locations[k].z ), "Boliches", 3.0, Config.Color, Config.LiquorBlipID)
    end
    ...
    for k, v in pairs(Config.Shops.MusiMundo.Locations) do
        CreateBlip(vector3(Config.Shops.MusiMundo.Locations[k].x, Config.Shops.MusiMundo.Locations[k].y, Config.Shops.MusiMundo.Locations[k].z ), "MusiMundo", 3.0, Config.Colormusimundo, Config.MusiMundoBlipID)
    end
    ...
    for k, v in pairs(Config.Shops.WeaponShop.Locations) do
        CreateBlip(vector3(Config.Shops.WeaponShop.Locations[k].x, Config.Shops.WeaponShop.Locations[k].y, Config.Shops.WeaponShop.Locations[k].z), "Ammu-nation", 3.0, Config.WeaponColor, Config.WeaponShopBlipID)
    end
end)
    ...
    while true do
        ...
        for k, v in pairs(Config.Shops.RegularShop.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.RegularShop.Locations[k].x, Config.Shops.RegularShop.Locations[k].y, Config.Shops.RegularShop.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.RegularShop.Locations[k].x, Config.Shops.RegularShop.Locations[k].y, Config.Shops.RegularShop.Locations[k].z + 1.0), "Presiona [E] para abrir la tienda", 0.6)
            end
        end
        ...
        for k, v in pairs(Config.Shops.RobsLiquor.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.RobsLiquor.Locations[k].x, Config.Shops.RobsLiquor.Locations[k].y, Config.Shops.RobsLiquor.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.RobsLiquor.Locations[k].x, Config.Shops.RobsLiquor.Locations[k].y, Config.Shops.RobsLiquor.Locations[k].z + 1.0), "Presiona [E] para abrir la tienda", 0.6)
            end
        end
        ...
        for k, v in pairs(Config.Shops.YouTool.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.YouTool.Locations[k].x, Config.Shops.YouTool.Locations[k].y, Config.Shops.YouTool.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.YouTool.Locations[k].x, Config.Shops.YouTool.Locations[k].y, Config.Shops.YouTool.Locations[k].z + 1.0), "Presiona [E] para abrir la tienda", 0.6)
            end
        end

        for k, v in pairs(Config.Shops.MusiMundo.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.MusiMundo.Locations[k].x, Config.Shops.MusiMundo.Locations[k].y, Config.Shops.MusiMundo.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.MusiMundo.Locations[k].x, Config.Shops.MusiMundo.Locations[k].y, Config.Shops.MusiMundo.Locations[k].z + 1.0), "Presiona [E] para abrir la tienda", 0.6)
            end
        end
        ...
        for k, v in pairs(Config.Shops.PrisonShop.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.PrisonShop.Locations[k].x, Config.Shops.PrisonShop.Locations[k].y, Config.Shops.PrisonShop.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.PrisonShop.Locations[k].x, Config.Shops.PrisonShop.Locations[k].y, Config.Shops.PrisonShop.Locations[k].z), "Presiona [E] para abrir la tienda", 0.6)
            end
        end
        ...
        for k, v in pairs(Config.Shops.WeaponShop.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.WeaponShop.Locations[k].x, Config.Shops.WeaponShop.Locations[k].y, Config.Shops.WeaponShop.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.WeaponShop.Locations[k].x, Config.Shops.WeaponShop.Locations[k].y, Config.Shops.WeaponShop.Locations[k].z + 1.0), "Presiona [E] para abrir la tienda", 0.6)
            end
        end
    end
end)
